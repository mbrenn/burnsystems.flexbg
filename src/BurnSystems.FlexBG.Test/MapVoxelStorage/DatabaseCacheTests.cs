...
namespace BurnSystems.FlexBG.Test.MapVoxelStorage
{
    ...
    public class DatabaseCacheTests
    {
        ...
        public void TestRetrievalFromCache()
        {
            ...
            var partition2 = new Partition(0, 1, 100);
            ...
            var partition1Loaded = cache.LoadPartition(0, 0);
            var partition2Loaded1 = cache.LoadPartition(0, 1);
            var partition2Loaded2 = cache.LoadPartition(0, 1);
            ...
            var notCachedPartition = database.LoadPartition(0, 1);
            ...
            var partition2Loaded3 = cache.LoadPartition(0, 1);
            Assert.That(partition2Loaded3.GetFieldType(5, 6, 500), Is.EqualTo(4));

            // Clears cache
            cache.StoreAndClearAll();
            var notCachedPartition2 = database.LoadPartition(0, 1);
        ...
        public void TestFillCache()
        {
            ...
            for (var n = 0; n < 10; n++)
            {
                partitions[n] = cache.LoadPartition(n, 2);
            ...
            for (var n = 0; n < 10; n++)
            {
                loadedPartitions[n] = cache.LoadPartition(n, 2);
